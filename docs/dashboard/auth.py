#!/usr/bin/env python3
"""
🔐 sticky auth guard — WebAuthn (passkey) passwordless authentication.

Why this exists
---------------
Before this, anyone who knew the dashboard's IP:port could drive it. That's a
...
Think of the registered passkey as your **device identity** ("id + wallet"):
  • the credential_id    → your account handle on this dashboard
  • the public key       → the lock the dashboard keeps
...
----
  1. FIRST RUN (admin enrollment):
     ...
     - From now on the dashboard is sealed.
  ...
  3. GUARD (every request):
     - The JWT is sent on WS connect (?token=) and on each /api/* call
       ...
       No valid token → 401, dashboard stays sealed.

Storage
-------
Credentials + the server's JWT signing secret live in a single JSON file
(`AUTH_STORE`, default ./.sticky_auth.json, chmod 600). Self-contained, no DB.

Env knobs
---------
  STICKY_AUTH_ENABLED   "true"/"false"  (default true)  — master switch
  STICKY_AUTH_STORE     path            (default ./.sticky_auth.json)
  STICKY_AUTH_RP_ID     relying-party id (default derived from request host)
  STICKY_AUTH_RP_NAME   display name    (default "sticky")
  STICKY_AUTH_ORIGIN    expected origin (default derived from request)
  STICKY_AUTH_TOKEN_TTL session seconds (default 86400 = 24h)
  STICKY_AUTH_BOOTSTRAP_TOKEN  optional one-time secret required to enroll
...
AUTH_ENABLED = _bool_env("STICKY_AUTH_ENABLED", True)
AUTH_STORE = Path(os.getenv("STICKY_AUTH_STORE", "./.sticky_auth.json")).resolve()
RP_NAME = os.getenv("STICKY_AUTH_RP_NAME", "sticky")
TOKEN_TTL = int(os.getenv("STICKY_AUTH_TOKEN_TTL", "86400"))
BOOTSTRAP_TOKEN = os.getenv("STICKY_AUTH_BOOTSTRAP_TOKEN", "").strip()

# RP_ID / ORIGIN can be forced via env; otherwise derived per-request from Host.
FORCE_RP_ID = os.getenv("STICKY_AUTH_RP_ID", "").strip()
FORCE_ORIGIN = os.getenv("STICKY_AUTH_ORIGIN", "").strip()
...
def delete_credential(cred_id: str) -> Dict[str, Any]:
    ...
    out and re-open the dashboard to anyone via the setup flow)."""
    store = _load()
...
def _extract_token(request: Request) -> Optional[str]:
    ...
    if auth.lower().startswith("bearer "):
        return auth[7:].strip()
    # cookie fallback
    tok = request.cookies.get("sticky_session")
    if tok:
        return tok
...
def begin_registration(request: Request, label: str = "admin passkey", bootstrap: str = "") -> Dict[str, Any]:
    ...
    gated by STICKY_AUTH_BOOTSTRAP_TOKEN to prevent a stranger grabbing the
    ...
    if not rpid_is_usable(rp_id):
        raise HTTPException(
            ...
            "Open the dashboard via a hostname instead (e.g. https://sticky.local:PORT "
            "or a /etc/hosts entry), or set STICKY_AUTH_RP_ID to a domain you control.",
        )
    ...
    opts = generate_registration_options(
        ...
        user_name="sticky-admin",
        user_display_name="Sticky Admin",
...
def begin_authentication(request: Request) -> Dict[str, Any]:
    ...
    if not rpid_is_usable(rp_id):
        raise HTTPException(
            400,
            f"WebAuthn cannot use '{rp_id}' (a raw IP) as the relying-party id. "
            "Open the dashboard via a hostname (https://sticky.local:PORT or a hosts "
            "entry), or set STICKY_AUTH_RP_ID to a domain.",
        )
    allow = [
        PublicKeyCredentialDescriptor(id=base64url_to_bytes(c["id"]))
        for c in store["credentials"]
        ...
        try:
            ...
            if not secure:
                ...
                out["warning"] = (f"'{host}' is a raw IP — WebAuthn needs a hostname/domain "
                                  "as rpId. Use https://<name>:PORT or set STICKY_AUTH_RP_ID.")
        except Exception:
            pass
    return out
